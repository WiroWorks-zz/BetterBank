...
function GetIBANFromIdentifier(targetID)
...
function agaSaatKac()
	...
	return (os.date("%m-%d-%Y ") ..hour ..":" ..minute)
end
...
function GetNameFromIBAN(IBAN)
	...
	local nameAndIBAN = result[1].firstname .." " ..result[1].lastname
	return nameAndIBAN
end
...
AddEventHandler('esx_billing:sendBill', function(targetId, sharedAccountName, label, amount)
	...
	local senderIBAN = GetIBANFromIdentifier(xPlayer.identifier)
	local targetIBAN = GetIBANFromIdentifier(xTarget.identifier)

	print(type(amount) .. amount)
	
	if amount > 0 and xTarget then
		MySQL.Async.execute("INSERT INTO billing (senderIBAN, senderName, targetIBAN, label, amount, neZaman) VALUES(@senderIBAN, @senderName, @targetIBAN, @label, @amount, @neZaman)",{
			['@senderIBAN'] = senderIBAN,
			['@senderName'] = GetNameFromIBAN(senderIBAN),
